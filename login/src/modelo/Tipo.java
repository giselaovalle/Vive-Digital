...
public class Tipo {
    ...
    ArrayList<Tipo> losTipos = new ArrayList<>();
        ...
        try {
            PreparedStatement sql = Conexion.conexion.prepareStatement("SELECT * FROM tipo");
    ...
    {
        try {
            PreparedStatement sql = Conexion.conexion.prepareStatement("INSERT INTO `tipo`( `descripcionTipo`) VALUES (?)");
    ...
    {
        try {
            PreparedStatement sql = Conexion.conexion.prepareStatement("UPDATE `tipo` SET `descripcionTipo`=?");
            PreparedStatement auditoria = Conexion.conexion.prepareStatement("INSERT INTO `auditoria`(`descripcionTipoAuditoria`) VALUES ('Se Modificó descripcion tipo',null)");
    ...
    {
        try {
            PreparedStatement sql = Conexion.conexion.prepareStatement("DELETE FROM `tipo` WHERE `idTipo`=?");
            PreparedStatement auditoria = Conexion.conexion.prepareStatement("INSERT INTO `auditoria`(`descripcionTipoAuditoria`) VALUES ('Se Eliminó Cliente',null)");
            auditoria.executeUpdate();
    ...
    ArrayList<Tipo> losTipos = new ArrayList<>();
        try {
            PreparedStatement sql = Conexion.conexion.prepareStatement("SELECT * FROM `tipo` WHERE `idTipo` LIKE? OR `descripcionTipo` LIKE?");
   ...
   unTipo.setDescripcionTipo("el tipo no existe");
       try {
           PreparedStatement sql = Conexion.conexion.prepareStatement("SELECT * FROM `tipo` WHERE `idTipo`=?");
           sql.setInt(1, elId);
           ResultSet rs = sql.executeQuery();
           while (rs.next()) {  
               unTipo.setIdTipo(     rs.getInt("idTipo"));
